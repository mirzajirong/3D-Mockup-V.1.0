...
export const BackgroundPanel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-3 p-4 text-white overflow-y-auto max-h-full font-geist">
      {/* Mode Switch Tabs (Checker tab removed; default remains checkerboard) */}
      <div className="bg-black/35 backdrop-blur-md rounded-[4px] border border-white/10 p-3 space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold text-white">Backdrop Mode</span>
          {scene.backgroundType === 'checkerboard' ? (
            <span className="text-[10px] text-[#DB0B2B] font-mono bg-[#251013] border border-[#DB0B2B]/40 px-2 py-0.5 rounded-[2px]">
              Default Papan Catur
            </span>
          ) : (
            <button
              ...
              className="flex items-center gap-1 text-[10px] text-[#888888] hover:text-white transition-colors cursor-pointer"
        ...
        <div className="grid grid-cols-3 gap-1 bg-black/40 p-1 rounded-[4px] border border-white/10">
          <button
            type="button"
        ...
        {scene.backgroundType === 'checkerboard' && (
          <div className="space-y-2 pt-1">
            <div className="flex items-center gap-2.5 p-2.5 bg-white/5 rounded-[4px] border border-white/10">
              <div
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-semibold text-white">Default Papan Catur Gelap</div>
        ...
        {scene.backgroundType === 'solid' && (
          <div className="space-y-3 pt-1">
            <div className="flex items-center justify-between">
              <span className="text-xs text-[#aaaaaa]">Color Value</span>
            ...
            <div className="space-y-1.5">
              <span className="text-[11px] text-[#777777]">Studio Color Presets</span>
        ...
        {/* 2. Custom Gradient with Angle & Custom Colors (No presets) */}
        {scene.backgroundType === 'gradient' && (
          <div className="space-y-3 pt-1">
            {/* Live Gradient Preview swatch */}
            <div
              className="w-full h-14 rounded-[4px] border border-white/20 shadow-inner"
              style={{
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                ...
                <label className="relative flex items-center gap-2 bg-white/5 hover:bg-white/10 px-2.5 py-1.5 rounded-[4px] border border-white/10 cursor-pointer transition-colors">
                  <span
                    className="w-4 h-4 rounded-[2px] border border-white/20 shrink-0"
                    style={{ backgroundColor: color1 }}
                  />
                  <span className="font-mono text-xs text-white uppercase tracking-wider truncate">
                    {color1}
                  </span>
                  <input
                    type="color"
                  ...
                  <button
                    ...
                    className={`py-1 text-[11px] font-mono rounded-[3px] border transition-colors cursor-pointer ${
                      angle === deg
                        ...
                        : 'bg-white/5 text-[#888888] border-white/10 hover:text-white hover:border-white/30'
                    }`}
                  >
                    {deg}°
                  </button>
          ...
          <div className="space-y-3 pt-1">
            ...
            {scene.backgroundImageUrl ? (
              <div className="space-y-2">
                <div className="relative w-full h-28 rounded-[4px] border border-white/15 overflow-hidden bg-black/40">
                  <img
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 p-1.5 bg-black/70 hover:bg-[#DB0B2B] text-white rounded-[3px] border border-white/20 transition-colors cursor-pointer"
                    title="Remove Image"
                  >
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 bg-white/10 hover:bg-white/15 border border-white/15 rounded-[4px] text-xs font-medium text-white transition-colors cursor-pointer backdrop-blur-md"
                >
              ...
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                className="w-full border-2 border-dashed border-white/15 hover:border-[#DB0B2B] hover:bg-white/5 rounded-[4px] py-6 flex flex-col items-center justify-center gap-2 text-[#888888] hover:text-white transition-all cursor-pointer backdrop-blur-md"
              >
                <div className="w-8 h-8 rounded-[4px] bg-white/5 flex items-center justify-center">
                  <ArrowUpTrayIcon className="w-4 h-4 text-[#DB0B2B]" />
                </div>
                <div className="text-center">
                  <div className="text-xs font-medium text-white">Upload Background Image</div>
                  <div className="text-[10px] text-[#777777]">PNG, JPG, WebP up to 10MB</div>
                </div>
              </button>
            )}
          </div>
